...
module.exports = function(inputAddress) {
  return new Promise((resolve, reject) => {
    ...
    .then( ({body}) => {
      ...
      if (geoResponse.number) resolve(geoResponse);
      reject(geoResponse);
    })
    .catch(reject);
  });
...
function handleGeoResponse(locationString) {
  const {
    results:[{
      address_components,
      formatted_address: fullAddress,
      geometry: location, place_id, types: type
    }],
    status
  } = locationString;
